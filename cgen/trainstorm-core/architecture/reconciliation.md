...
3. **The one critical fix:** `course-primitives` embeds every translation inline (`content: localizedText` with en/fr/de/es…). That is the exact drift-generator you lived through on AST009. Externalize it — keep the source string in the element, move other locales to packs keyed by `element_id`.
...
- **`course-primitives`' flat, ID-keyed element array** → this *is* the atom store. Make `element_id` the stable `atom_id`.
...
1. **Externalize localization (the critical one).** `content: localizedText` embeds en/fr/de/es inline — the AST009 drift-generator. Fix: keep `content` in the source locale as the element's **meaning**; move every other locale to **locale packs keyed by `element_id`**, each with `status` / `reviewer` / `source_hash`. (This is exactly the atom spec's §4. It's also what lets your localization agent — proven on AST009 — plug straight in.)
...
```jsonc
{
  "element_id": "ele_ast009_recognize_psi",     // = atom_id (stable, the join key)
  ...
  "structure": {                                 // object facet
    ...
    "parent_id": "ele_slide_1_3", "sequence_index": 4 },
...
```
Translations for this element live in `locales/<lang>.json[element_id]`; the visual/motion/interaction primitives live in their registries; objectives live in the intent ontology. The element stays thin.
