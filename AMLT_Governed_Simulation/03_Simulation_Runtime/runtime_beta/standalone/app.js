const PROXY_HEALTH_URL = "http://localhost:3000/health";
...
async function checkProxy() {
  ...
  try {
    ...
    el.textContent = "❌ Not running (start: node server.js)";
  }
}
...
// ===== CONFIG (local proxy) =====
const ENDPOINT_URL = "http://localhost:3000/run-amlt";
const PROMPT_VERSION = "AMLT_SIM_v4.2.1";
...
function validateGovernanceJson(g) {
  ...
  if (!g) errors.push("No JSON parsed.");
    ...
    const requiredTop = [
      "simulation_id",
      "asset_id",
      "competency_model_id", // v4.2.1 placeholder support
      "round",
      "simulation_master",
      "decision_node",
      "completion",
...
// ===== Main =====
document.getElementById("checkProxyBtn").addEventListener("click", checkProxy);
...
document.getElementById("runBtn").addEventListener("click", async () => {
  ...
  const proxyText = document.getElementById("proxyStatus")?.textContent || "";
  if (!proxyText.includes("✅")) {
    setStatus("❌ Proxy not connected. Start the proxy: cd proxy_server && node server.js");
    return;
  }
  ...
  try {
    const res = await fetch(ENDPOINT_URL, {
      ...
      // body: JSON.stringify({ learner_decision_payload: payload.learner_decision_payload, prior_round_governance_json: null, prompt_version: PROMPT_VERSION })
    });
    ...
    if (v.ok) {
      setStatus("✅ Governance validated. (Local proxy)");
